...
const SessionChart = ({ timeRanges, session }) => {
  ...
  /**
   * Creates Session chart svg elements with d3
   * @param {Object[]} data
   * @param {int} columns
   */
  const drawBarChart = (data, columns) => {
    ...
    const barGap = 5;
    const barWidth = width / (columns + 1) - barGap;
    ...
    const setUpRects = () => {
      ...
      svgCanvas
        ...
        .attr("fill", (datapoint, iteration) =>
          barColors[datapoint.status] ? barColors[datapoint.status] : "purple"
        )
        .attr(
          "x",
          (datapoint, iteration) =>
            datapoint.index * (barWidth + barGap) + margin.left + barGap
        )
      ...
      svgCanvas.append("g").attr("class", "rectinfo");
    ...
    // Sets up current time line on hover over chart
    ...
    /**
     * Updates svg crrent time line every interval
     * @returns undefined
     */
  ...
  /**
   * Processes time ranges before drawing ranges on chart
   * @returns object: ranges (array), columns (int)
   */
  const processTimeRanges = () => {
    let ranges = [];

    // To 'stack' user ranges, keep track of a users' 'column'
    // Key: userid, Value: Index/Column
    const userRangeMap = new Map();
    let columnCount = 0;
    ...
    timeRanges.map((range) => {
      ...
      let index;
      const mapValue = userRangeMap.get(range.user_id);

      // CHeck if userId in map
      if (mapValue !== undefined) {
        index = mapValue;
      } else {
        userRangeMap.set(range.user_id, columnCount);
        index = columnCount;
        columnCount++;
      }
      ranges.push({
    ...
    return { ranges: ranges, columns: columnCount };
  };
